...
export const Overview = () => {
  ...
  const fetchOrderCount = useCallback(
    async (
      ...
      const { data } = await axios.post<OrderCountResponse>(
        '/api/orders/count',
        requestData ?? {},
        {
          withCredentials: true,
  ...
  return (
    <section className="flex flex-col gap-6">
      <div className="twp grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        <OverviewCard
          label="Total Revenue"
          icon={<DollarSign className="h-5 w-5 text-success opacity-75" strokeWidth={3} />}
          value={currency ? formatPrice(totalRevenue.value, currency) : totalRevenue.value}
          percentage={totalRevenue.percentage}
        />
        <OverviewCard
          label="Total Orders"
          icon={<ClipboardPaste className="h-5 w-5 font-bold opacity-75" />}
          value={totalOrders.value}
          percentage={totalOrders.percentage}
        />
        <OverviewCard
          label="Ranged Revenue"
          icon={
            <DollarSign className="h-5 w-5 text-success font-bold opacity-75" strokeWidth={3} />
          }
          value={currency ? formatPrice(rangedRevenue.value, currency) : rangedRevenue.value}
