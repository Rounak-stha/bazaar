...
export const DefaultHeader = ({ data, disableCart }: { data: Header; disableCart?: boolean }) => {
  ...
  const isRoot = useMemo(() => pathname == "/en", [pathname]);
  ...
  const classes = cn(
    `flex w-full top-0 justify-center md:px-12 transition-transform z-50`,
  ...
  return (
    <header className={classes} style={data.background ? { background: data.background } : {}}>
      <div
        className={`relative container flex w-full items-center py-6 lg:gap-8 ${scrolled ? "scrolled" : ""} ${isMenuOpened ? "opened" : ""}`}
      >
        <Link href="/" className="mr-auto text-2xl font-medium lg:mr-0">
          BagBae.
          {/* {data.logo && typeof data.logo !== "number" && data.logo.url && data.logo.alt ? (
            <Media
              resource={data.logo}
            ...
            {data.navItems?.map(({ link }, i) => {
              ...
              return (
                <CMSLink
                  ...
                  {...link}
                  appearance="link"
                  className={`${scrolled ? "text-black" : "text-white"} text-base`}
                />
              );
            })}
          </div>
